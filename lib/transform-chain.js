		...
		return new Promise((resolved, rejected) => {
			this.cacheChain.get(localFilename).then((result) => {
				...
				async.forEachOfSeries(renderChain, (item, idx, next) => {
					...
					if (lastResult.map) {
						lastResult.data = new Buffer(lastResult.data.toString().replace(/sourceMappingURL=[.*?][ $]/, 'sourceMappingURL=' + path.basename(remoteFilename) + '.min'));
					}
