...
class TransformChain {
	...
	render(localFilename, remoteFilename, options) {
		return new Promise((resolved, rejected) => {
			this.cacheChain.get(localFilename, options).then((result) => {
				...
				this._buildChain(localFilename, remoteFilename, renderChain => {
					async.waterfall(renderChain.map(item => {
						return (lastResult, next) => {
							...
							item.transform.compile(item.src.local, lastResult.data || item.src.local, lastResult.map, options)
							.then(itemResult => {
						...
						this.cacheChain.set(localFilename, options, lastResult).then(() => {
