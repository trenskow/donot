 ...
 */
 var SmartStatic = function(root, opt) { var self = this;
   ...
   if (!(self instanceof SmartStatic)) return new SmartStatic(root, serveDir, opt);
  ...
  if (self.options.serveDir.substr(-1) !== '/') {
    self.options.serveDir += '/';
  }
 ...
 * @api private
 */
SmartStatic.prototype.testAccess = function(file) { var self = this;
  ...
  for (var idx in list) {
    ...
    if (typeof item === 'string') {
      if (item.toLowerCase() == fullextname(file).toLowerCase()) return allows;
    }

    // Regular expression
    else if (item.test(file)) {
      return allows;
    }
  }
...
SmartStatic.prototype.remoteToLocalPath = function(u) { var self = this;

  var pathname = url.parse(u).pathname;

  // Make sure pathname is in our serveDir
  if (pathname.substr(0, self.options.serveDir.length) !== self.options.serveDir) {
    return false;
  }
