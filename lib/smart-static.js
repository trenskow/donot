    ...
    async.forEachOfSeries(engine.map, function(templateExt, targetExt, nextExt) {
      ...
      fs.exists(templateFile, function(exists) {
        ...
        compile(engine, templateFile, function(err, data, cached) {
          ...
          engine.render(url, data, 'utf8', function(err, source) {
...
function smartStatic(root, opt) {
  ...
  options.engines = options.engines || [];
  ...
  return function route(req, res, next) {
    ...
    fs.exists(file, function(exists) {
      ...
      // File does not exist
      if (!exists) {
        return render(req.url, function(err, data) {
...
function engine(opt) {
  ...
  if (!opt.compiler || typeof opt.compiler !== 'function') {
    throw new TypeError('engine must have a compiler and it must be a function');
  }

  if (opt.renderer !== undefined && typeof opt.renderer !== 'function') {
    throw new TypeError('engine renderer must be a function');
  }

  // Copy options
  var engine = merge(opt, {});

  // Set a defaults
  engine.renderer = engine.renderer || function(url, source, encoding, cb) {
    cb(null, source);
  };
  engine.encoding = engine.encoding || 'utf8';

  // We just add the engine
