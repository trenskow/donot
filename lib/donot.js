...
class Donot {
	...
	constructor(root, opt) {
		...
		this.options.sourceMaps = opt.sourceMaps !== false;
	...
	render(remoteFilename, ctx) {
		return new Promise((resolved, rejected) => {
			...
			(new TransformChain(this.transforms, this.cacheChain, this.options))
			    .render(utils.remoteToLocalFilename(remoteFilename, this.root, this.options.serveDir),
					        remoteFilename,
					        ctx)
	...
	route(req, res, next) {
		...
		fs.exists(localFilename, (exists) => {
			...
			if (!exists) {
				...
				var sourceMapTester = /\.map$/i;
				var sourceMap = sourceMapTester.test(localFilename);
				if (sourceMap) {
					if (this.options.sourceMaps === true) {
						localFilename = localFilename.replace(sourceMapTester, '');
						remoteFilename = remoteFilename.replace(sourceMapTester, '');
					} else {
						return next();
					}
				}

				return this.render(remoteFilename, (req.donot || {}).locals).then((result) => {

					// If no result - next route
					if (!result) return next();

					// If source map requested - make data source map
					if (sourceMap) {
						result.data = result.map ? new Buffer(JSON.stringify(result.map)) : null;
						result.filename += '.min';
					}
